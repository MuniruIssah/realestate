...
const UnderConstruction = () => {
  return (
    <div className="w-full flex flex-col justify-center items-center space-y-10 p-10">
      ...
      <button className="bg-teal-100 text-sm rounded rounded-full text-teal-600 p-2  self-end shadow-md">
        <MdOutlineArrowForwardIos />
      </button>
      <button className="text-white bg-teal-600 py-2 px-3 rounded rounded-md">
        Tap for Free book
      </button>
    </div>
  );
...
const UnderConstructionCard = ({ image, service, description }) => {
  return (
    <div className="group snap-start flex-shrink-0  md:w-60 border border-teal-200 hover:bg-teal-600 hover:ring-teal-600 rounded rounded-xl">
      <img
        src={image}
        alt="service"
        className="w-full h-36 object-cover rounded rounded-xl group-hover:contrast-50"
      />
      <span className="block py-3 px-2 text-gray-600 font-semibold group-hover:text-white ">
        {service}
      </span>
      <span className="block pb-3 px-2 text-gray-400 text-sm  group-hover:text-white ">
        {description}
      </span>
